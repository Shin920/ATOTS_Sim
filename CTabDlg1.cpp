...
CTabDlg1::CTabDlg1(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DLG_TAB1, pParent)
{
	InitVariables();
	Init();
...
BEGIN_MESSAGE_MAP(CTabDlg1, CDialogEx)
	...
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB_TS, &CTabDlg1::OnTcnSelchangeTabTs)
	ON_WM_ERASEBKGND()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BTN_INIT, &CTabDlg1::OnBnClickedBtnInit)
	ON_BN_CLICKED(IDC_BTN_APPLY, &CTabDlg1::OnBnClickedBtnApply)
	//ON_BN_CLICKED(IDC_CHECK_AUTO, &CTabDlg1::OnBnClickedCheckAuto)
	ON_BN_CLICKED(IDC_BUTTON_START, &CTabDlg1::OnBnClickedButtonStart)
	ON_BN_CLICKED(IDC_BUTTON_FINISH, &CTabDlg1::OnBnClickedButtonFinish)
	ON_NOTIFY(NM_CLICK, IDC_LIST_MSG, &CTabDlg1::OnNMClickListMsg)
	ON_BN_CLICKED(IDC_BUTTON_CON, &CTabDlg1::OnBnClickedButtonCon)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON_DISCON, &CTabDlg1::OnBnClickedButtonDiscon)
	ON_NOTIFY(LVN_BEGINLABELEDIT, IDC_LIST_MSG, &CTabDlg1::OnLvnBeginlabeleditListMsg)
	ON_WM_CONTEXTMENU()
	ON_MESSAGE(WM_INSERTPACKET, &CTabDlg1::LogFileAdd)
	ON_NOTIFY(NM_RCLICK, IDC_LIST_MSG, &CTabDlg1::OnRclickListMsg)
	ON_COMMAND(IDR_MENU_EXPAND, &CTabDlg1::OnMenuExpand)
	ON_COMMAND(IDR_MENU_ALL_DELETE, &CTabDlg1::OnMenuAllDelete)
	ON_WM_DESTROY()
	ON_CBN_SELCHANGE(IDC_CMB_RECV, &CTabDlg1::OnCbnSelchangeCmbRecv)
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_LIST_MSG, &CTabDlg1::OnLvnItemchangedListMsg)
	ON_EN_KILLFOCUS(IDC_cEN, &CTabDlg1::OnEnKillfocuscen)
	ON_EN_KILLFOCUS(IDC_cOP, &CTabDlg1::OnEnKillfocuscop)
	ON_EN_KILLFOCUS(IDC_cMAV, &CTabDlg1::OnEnKillfocuscmav)
	ON_BN_CLICKED(IDC_BUTTON_ADD, &CTabDlg1::OnBnClickedButtonAdd)
	ON_BN_CLICKED(IDC_BUTTON_CLEAR, &CTabDlg1::OnBnClickedButtonClear)
	ON_BN_CLICKED(IDC_CHECK_INDI, &CTabDlg1::OnBnClickedCheckIndi)
END_MESSAGE_MAP()
...
void CTabDlg1::Init_Font()
{
	...
	FontManager::GetInstance().InsertFont(_T("sansBold17"), tempFont);
	...
	FontManager::GetInstance().InsertFont(_T("sansThin17"), tempFont);
...
BOOL CTabDlg1::OnInitDialog()
{
	...
	int num = 101;
	//p07Tab->RequestedLoadSPFromFile(num);

	m_ras_tab.InsertItem(eHsAck, _T("HSAck"));
	m_ras_tab.InsertItem(eHsRej, _T("HSRej"));
	m_ras_tab.InsertItem(eJP, _T("JP"));
	m_ras_tab.InsertItem(eSP, _T("SP"));
	m_ras_tab.InsertItem(eSTRAck, _T("STRAck"));
	m_ras_tab.InsertItem(eSeTrmReq, _T("STReq"));

	m_ras_tab.SetCurSel(eHsAck);
	...
	p01Tab = new packet01Tab;
	p01Tab->Create(IDD_DLG_SEND_1, &m_ras_tab);
	p01Tab->MoveWindow(0, 25, rect.Width(), rect.Height());
	p01Tab->ShowWindow(SW_SHOW);

	p02Tab = new packet02Tab;
	p02Tab->Create(IDD_DLG_SEND_2, &m_ras_tab);
	p02Tab->MoveWindow(0, 25, rect.Width(), rect.Height());
	p02Tab->ShowWindow(SW_HIDE);

	p04Tab = new packet04Tab;
	p04Tab->Create(IDD_DLG_SEND_3, &m_ras_tab);
	p04Tab->MoveWindow(0, 25, rect.Width(), rect.Height());
	p04Tab->ShowWindow(SW_HIDE);

	p07Tab = new packet07Tab;
	p07Tab->Create(IDD_DLG_SEND_4, &m_ras_tab);
	p07Tab->MoveWindow(0, 25, rect.Width(), rect.Height());
	p07Tab->ShowWindow(SW_HIDE);

	p09Tab = new packet09Tab;
	p09Tab->Create(IDD_DLG_SEND_5, &m_ras_tab);
	p09Tab->MoveWindow(0, 25, rect.Width(), rect.Height());
	p09Tab->ShowWindow(SW_HIDE);
	
	p10Tab = new packet10Tab;
	p10Tab->Create(IDD_DLG_SEND_6, &m_ras_tab);
	p10Tab->MoveWindow(0, 25, rect.Width(), rect.Height());
	p10Tab->ShowWindow(SW_HIDE);
	...
	myPath.Format(_T("%s\\Config.ini"), Utility::GetExePath(g_criticalExe_1));
	SPPath.Format(_T("%s\\JP, SP ini\\SP\\"), Utility::GetExePath(g_criticalExe_1));
	...
	CString tsId = Utility::INIReadString("TS-01", "TS ID", myPath);
	CString tsIp = Utility::INIReadString("TS-01", "IP", myPath);
	CString tsPort = Utility::INIReadString("TS-01", "PORT", myPath);
	CString obIp = Utility::INIReadString("TS-01", "OB IP", myPath);
	CString obPort = Utility::INIReadString("TS-01", "OB PORT", myPath);
	...
	CString op = Utility::INIReadString("TS-01", "NID_OPERATIONAL", myPath);
	CString en = Utility::INIReadString("TS-01", "NID_ENGINE", myPath);
	CString ver = Utility::INIReadString("TS-01", "VERSION", myPath);
...
#pragma endregion
	...
	//m_strList.SetItemText(0, 1, _T("Conditions fullfil"));

	//Recv Porpertygrid

	m_cmb_recv.AddString("Receive : HandShake Request Packet");
	m_cmb_recv.AddString("Test : HandShake Acknowledgement Packet");
	m_cmb_recv.AddString("Test : HandShake Reject Packet");
	m_cmb_recv.AddString("Receive : Journey Profile Request Packet");
	m_cmb_recv.AddString("Test : Journey Profile Packet");
	...
	m_cmb_recv.AddString("Test : Segment Profile Packet");
	m_cmb_recv.AddString("Receive : Status Report Packet");
	m_cmb_recv.AddString("Test : Status Report Ack Packet");
	m_cmb_recv.AddString("Receive : Session Termination Packet");
	m_cmb_recv.AddString("Receive : HanShake Acknowledgement Packet");
	m_cmb_recv.SetCurSel(eHsAck);
...
BOOL CTabDlg1::OnEraseBkgnd(CDC* pDC)
{
	...
	while (pChild != nullptr)
	{
		...
		pDC->FillSolidRect(childRect, RGB(255, 255, 255));
...
HBRUSH CTabDlg1::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	//DDX_Control(pDX, IDC_ALIVE_COUNTER, m_send_alive);
	//DDX_Control(pDX, IDC_RECV_ALIVE_COUNTER, m_receive_alive);
	if (IDC_ALIVE_COUNTER == CTLCOLOR_STATIC && pWnd->GetDlgCtrlID() == IDC_STATIC)
	{
...
void CTabDlg1::OnBnClickedBtnInit()
{
...
void CTabDlg1::OnBnClickedBtnApply()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	...
	// INI 파일 덮어쓰기
	Utility::INIWriteString("TS-01", "TS ID", newID, myPath);
	Utility::INIWriteString("TS-01", "IP", newIp, myPath);
	Utility::INIWriteString("TS-01", "PORT", newPort, myPath);
	Utility::INIWriteString("TS-01", "OB IP", newOBIp, myPath);
	Utility::INIWriteString("TS-01", "OB PORT", newOBPort, myPath);
		
		
}
void CTabDlg1::Parse_Cmd_TS(BYTE* buf, int len)
{
	BOOL isRight = TRUE;
	int nCurrSeq = 0;
	...
	if (len < 10) {
		Debug("[received buffer Error] length is too short(%d)", len);
	...
	if (msg_struct.header.stx != 0x02) {
		Debug("[received buffer Error] stx value is wrong(0x%x)", msg_struct.header.stx);
	...
	if (nLength != len) {
		Debug("[received buffer Error] length is not same(header.len=%d, received buf len=%d)", msg_struct.header.length, len);
		MessageLog(_T("Error"), csCommand, _T("length"), buf, len);
		return;
	}
	...
	if (m_checkindi.GetCheck() == BST_CHECKED) {
		...
		case OPCODE_CONNECT_INDICATION:
			...
			if (m_nTS_ID != KR37_ByteToInt(msg_struct.body.connect_indi.responder.etcs_id)) {
				MessageLog_SC(_T("Receive"), _T("ETCS ID"), buf, len);
				break;
			}
			//INDICATION에는 SessionID 포함되어있음
			memcpy(m_SessID, msg_struct.body.connect_indi.sacepid, sizeof(m_SessID));
			m_nETCS_ID = KR37_ByteToInt(msg_struct.body.connect_indi.initiator.etcs_id);
			...
			if (m_auto_sa.GetCheck() == BST_CHECKED) {
				Cmd_Send(1);
		...
		case OPCODE_DATA_INDICATION:
			...
			csCommand = _T("Data.indi");

			// 헤더만 Decode 후 NID_PACET_ATO 구하기
			DecodeMessage_ATOHeader(msg_struct.body.data.data, LEN_ALL_HEADER, offset, pHeaderStructure);
			...
			if (msgType == aHSReq) {
				...
				// 자동 전송일 경우 
				if (p01Tab->m_chb_auto.GetCheck() == BST_CHECKED) {
					bool anyMatch = false;
					...
					for (int i = 0; i < pHSReqStructure->N_ITER; ++i) {
						if (pHSReqStructure->M_ATO_Version[i] == myVersion) {
							anyMatch = true;
							// HS Ack
							p01Tab->PostMessage(WM_COMMAND, MAKEWPARAM(IDC_BTN_SEND, BN_CLICKED), (LPARAM)GetDlgItem(IDC_BTN_SEND)->GetSafeHwnd());
							break; // 찾으면 break;
						}
					}
					...
					if (!anyMatch) {
						...
						// 0 : 시스템 비호환 
						tempGrid = p02Tab->m_propgrid.GetProperty(BODY_DATA);
						currentProperty = tempGrid->GetSubItem(IDX_Q_Reject_Reason);
						Utility::SetValueWithTypeCasting(currentProperty, 0);
						p02Tab->PostMessage(WM_COMMAND, MAKEWPARAM(IDC_BTN_SEND, BN_CLICKED), (LPARAM)GetDlgItem(IDC_BTN_SEND)->GetSafeHwnd());
					}
				}
			}
				...
				// 자동 전송일 경우
				if (p04Tab->m_chb_auto.GetCheck() == BST_CHECKED) {
					p04Tab->PostMessage(WM_COMMAND, MAKEWPARAM(IDC_BTN_SEND, BN_CLICKED), (LPARAM)GetDlgItem(IDC_BTN_SEND)->GetSafeHwnd());
				}
			}
				...
				// 자동 전송일 경우
				if (p07Tab->m_chb_auto.GetCheck() == BST_CHECKED && pSPReqStructure->N_ITER_ATO > 0) {
					for (int i = 0; i < static_cast<int>(pSPReqStructure->N_ITER_ATO); i++) {
						...
						if (it != spMap.end()) {
							index = it->second;
							p07Tab->RequestedLoadSPFromFile(index, SPPath); // Load
							p07Tab->PostMessage(WM_COMMAND, MAKEWPARAM(IDC_BTN_SEND, BN_CLICKED), (LPARAM)GetDlgItem(IDC_BTN_SEND)->GetSafeHwnd()); // send
						}
					}
				}
			}
				...
				// 자동 전송일 경우
				if (p09Tab->m_chb_auto.GetCheck() == BST_CHECKED) {
					p09Tab->PostMessage(WM_COMMAND, MAKEWPARAM(IDC_BTN_SEND, BN_CLICKED), (LPARAM)GetDlgItem(IDC_BTN_SEND)->GetSafeHwnd());
				}
			}
		...
		case OPCODE_DISCONNECT_INDICATION: // Disconnect req 처리 (!= finish)
			...
			memset(m_SessID, 0, sizeof(m_SessID)); // 모든 요소를 0으로 초기화
	...
	//기존 코드 Opcode
		...
		case OPCODE_CONNECT_REQUEST:
			...
			m_nETCS_ID = KR37_ByteToInt(msg_struct.body.connect_req.initiator.etcs_id);
		...
		case OPCODE_KVC_ALIVE: // 통신 테스트용 KVC ALIVE
			...
			rcnt++;
			break;
		case OPCODE_DISCONNECT_REQUEST: // Disconnect req 처리 (!= finish)
...
int CTabDlg1::KR37_GetLength(BYTE* bID)
{
	int retValue = 0;
...
void CTabDlg1::MessageLog(CString inout, CString cmd, CString type, CString origin_msg)
{	
	int nColumn = 0;
...
void CTabDlg1::MessageLog(CString inout, CString cmd, CString type, BYTE* buffer, int len)
{
...
void CTabDlg1::MessageLogVector(CString inout, CString cmd, CString type, const std::vector<UINT8>& buffer)
{
	CString csBody;
...
void CTabDlg1::MessageLog_SC(CString inout, CString cmd, CString origin_msg)
{
	int nColumn = 0;
	int	nIndex = m_lc_msg.GetItemCount();
	m_lc_msg.InsertItem(nIndex, "");
...
void CTabDlg1::MessageLog_SC(CString inout, CString cmd, BYTE* buffer, int len)
{
	CString csStx, csLen, csSeq, csOp, csBody;
	CString  hexPrint;
...
void CTabDlg1::SetSessionStatus(int index, BYTE state)
{
	SetSessionStatus(index, 0, 0, state);
}

void CTabDlg1::propgrid()
{
	CRect rect;
...
void CTabDlg1::chgCmb(int index)
{
	switch (index)
	{
	case aHSReq:					// NID_PACKET_ATO : 0 = HandShake Req
		...
		break;
	case aHSAck:					// NID_PACKET_ATO : 1 = HandShake Acknowledgement
		m_cmb_recv.SetCurSel(1);

		break;
	case aHSRej:					// NID_PACKET_ATO : 2 = HandShake Reject
	...
	case aJPReq:					// NID_PACKET_ATO : 3 = Journey Profile Req
		...
		break;
	case aJP:					// NID_PACKET_ATO : 4 = Journey Profile Req
	...
	case aSPReq:					// NID_PACKET_ATO : 6 = Segment Profile Req
		m_cmb_recv.SetCurSel(6);
		
		break;
	case aSP:					// NID_PACKET_ATO : 7 = Segment Profile
		m_cmb_recv.SetCurSel(7);

		break;
	case aSTR:					// NID_PACKET_ATO : 8 = Status Report
		m_cmb_recv.SetCurSel(8);
		
		break;
	case aSTRAck:					// NID_PACKET_ATO : 8 = Status Report Ack
		m_cmb_recv.SetCurSel(9);

		break;
	case aSeTerm:					// NID_PACKET_ATO : 11 = Session Termination
		m_cmb_recv.SetCurSel(10);
		
		break;
	}
}

void CTabDlg1::OnCbnSelchangeCmbRecv()
{
...
void CTabDlg1::Release_Ctrl()
{
	
}

void CTabDlg1::updtPacket(_In_ const byte* arr, _In_ const size_t length)
{
	...
	case aHSAck:
	{
		TS_OB_Packet01_HSAckStruct p{};
		DecodeMessage_ATO_Packet01_HsAck(&p, offset, arr, (UINT32)length);
		PropertyCtrlBuilder::PacketHSAckStructureToCtrl(p, m_propgrid.GetProperty((int)LogProperty::BODY));
	}
	break;
	case aJPReq:
	{
	...
	case aJP:
	{
		TS_OB_Packet04_JPStruct p{};
		DecodeMessage_ATO_Packet04_JP(&p, offset, arr, (UINT32)length);
		PropertyCtrlBuilder::PacketJPStructureToCtrl(p, m_propgrid.GetProperty((int)LogProperty::BODY));
	}
	break;
	case aJPAck:
	{
	...
	case aSP:
	{
		TS_OB_Packet07_SPStruct p{};
		DecodeMessage_ATO_Packet07_SP(&p, offset, arr, (UINT32)length);
		PropertyCtrlBuilder::PacketSPStructureToCtrl(p, m_propgrid.GetProperty((int)LogProperty::BODY));
	}
	break;
	case aSTR:
	{
	...
	case aSTRAck:
	{
		TS_OB_Packet09_STRAckStruct p{};
		DecodeMessage_ATO_Packet09_STRAck(&p, offset, arr, (UINT32)length);
		PropertyCtrlBuilder::PacketSTRAckStructureToCtrl(p, m_propgrid.GetProperty((int)LogProperty::BODY));
	}
	break;
	case aSeTerm:
	{
...
void CTabDlg1::SetCtrl_ATOstatus(ImgATOstatus v)
{
	switch (v)
	{
...
CString CTabDlg1::SetText_STRIndcators(UINT16 value)
{
	CString str;
...
void CTabDlg1::SetCtrl_TPinformation()
{	
...
void CTabDlg1::SessionSet(BYTE* sessID, BYTE* resp_etcsID)
{
	int nIndex = -1;
	CString csTmp;
...
void CTabDlg1::Cmd_Send(int cmd)
{
	/*
	...
	*/ 
	...
	case 1:
		if (m_checkindi.GetCheck() == BST_CHECKED) {
			...
			Utility::makeSessionID(m_SessID);				   // 단독 STU 통신 아닐 시 세션ID 직접 생성
		...
		memcpy(msg_struct.body.connect_resp_conf.sacepid, m_SessID, 3);
...
void CTabDlg1::Cmd_Send126(int type)
{
	SA_MESSAGE_STRUCTURE msg_struct;
	CString		csCommand;
	CString		csType;
	...
	**/
	...
	case eHsAck:
		...
		length = GetLengthPacket01_HsAck(p01Tab->pHSAckStructure);
		...
		p01Tab->pHeaderStructure->L_PACKET_ATO = GetLengthPacket01_HsAck(p01Tab->pHSAckStructure);
		Encode_ATOHeader(p01Tab->pHeaderStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));
					
		EncodeMessage_ATO_Packet01_HsAck(p01Tab->pHSAckStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));
	...
	case eHsRej:
		...
		length = GetLengthPacket02_HsRej(p02Tab->pHSRejStructure);
		bodylength = length / BITS_OF_ONEBYTE;
		...
		//p02Tab->pHeaderStructure->L_PACKET_ATO = GetLengthPacket02_HsRej(p02Tab->pHSRejStructure);

		p02Tab->pHeaderStructure->L_PACKET_ATO = GetLengthPacket02_HsRej(p02Tab->pHSRejStructure);

		Encode_ATOHeader(p02Tab->pHeaderStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));
		//TRACE1(_T("size of pHeader Structure : %d\n"), (int)sizeof(pSTab->pHeaderStructure));	

		EncodeMessage_ATO_Packet02_HsRej(p02Tab->pHSRejStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));
	...
	case eJP:
		...
		length = GetLengthPacket04_Jp(p04Tab->pJPStructure);
		bodylength = length / BITS_OF_ONEBYTE;
		...
		p04Tab->pHeaderStructure->L_PACKET_ATO = GetLengthPacket04_Jp(p04Tab->pJPStructure);
		Encode_ATOHeader(p04Tab->pHeaderStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));

		EncodeMessage_ATO_Packet04_Jp(p04Tab->pJPStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));
	...
	case eSP:
		...
		length = GetLengthPacket07_Sp(p07Tab->pSPStructure);
		bodylength = length / BITS_OF_ONEBYTE;
		...
		p07Tab->pHeaderStructure->L_PACKET_ATO = GetLengthPacket07_Sp(p07Tab->pSPStructure);
		Encode_ATOHeader(p07Tab->pHeaderStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));

		EncodeMessage_ATO_Packet07_Sp(p07Tab->pSPStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));
	...
	case eSTRAck:
		...
		length = GetLengthPacket09_STRAck(p09Tab->pSTRAckStructure);
		bodylength = length / BITS_OF_ONEBYTE;
		...
		p09Tab->pHeaderStructure->L_PACKET_ATO = GetLengthPacket09_STRAck(p09Tab->pSTRAckStructure);
		Encode_ATOHeader(p09Tab->pHeaderStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));

		EncodeMessage_ATO_Packet09_STRAck(p09Tab->pSTRAckStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));
	...
	case eSeTrmReq:
		...
		p10Tab->pHeaderStructure->L_PACKET_ATO = GetATOHeaderSize();
		Encode_ATOHeader(p10Tab->pHeaderStructure, &nPos, msg_struct.body.data.data, sizeof(msg_struct.body.data.data));
	...
	msg_struct.header.stx = 0x02;
	if (m_checkindi.GetCheck() == BST_CHECKED) {
		msg_struct.header.opcode = OPCODE_DATA_REQUEST; // 체크된 경우 사용할 opcode
	}
	else {
		msg_struct.header.opcode = OPCODE_DATA_INDICATION; // 기본 opcode
	}
	...
	//session id 구조체 저장
	memcpy(msg_struct.body.data.sacepid, m_SessID, BYTES_OF_SESSID);	// 3 Bytes
...
int CTabDlg1::SendTo(BYTE* buf, int len)
{	
	int nTmp;
...
void CTabDlg1::KR37_MakeLength(BYTE* bID, int length)
{
...
void CTabDlg1::KR37_IntToByte(int nID, BYTE* bID, int size)
{
	if (size == 3) {
...
int CTabDlg1::KR37_ByteToInt(BYTE* bID)
{
	int retValue = 0;
...
void CTabDlg1::Debug(CString msg)
{
...
void CTabDlg1::Debug(const char* format, ...)
{
...
CString CTabDlg1::Get_CurrentTime()
{
...
int CTabDlg1::SetSessionStatus(int index, BYTE* obID, BYTE* sessID, int state)
{
...
int CTabDlg1::CheckSessionID(BYTE* sessID)
{
	return 0;
}

void CTabDlg1::SetConfigList(BOOL isInsert, int row, CString csCategory, CString csValue)
{
	
}

void CTabDlg1::Init_IconImage()
{

	imgDisConn.LoadFromResource(AfxGetInstanceHandle(), IDB_DISCONNECT);
	imgConn.LoadFromResource(AfxGetInstanceHandle(), IDB_CONNECT);
	imgBlank.LoadFromResource(AfxGetResourceHandle(), IDB_BLANK);
	imgStatusNP.LoadFromResource(AfxGetResourceHandle(), IDB_ATO_NP);
	imgStatusCO.LoadFromResource(AfxGetInstanceHandle(), IDB_ATO_CO);
	imgStatusNA.LoadFromResource(AfxGetInstanceHandle(), IDB_ATO_NA);
	imgStatusAV.LoadFromResource(AfxGetInstanceHandle(), IDB_ATO_AV);
	imgStatusRE.LoadFromResource(AfxGetInstanceHandle(), IDB_ATO_RE);
	imgStatusEG.LoadFromResource(AfxGetInstanceHandle(), IDB_ATO_EG);
	imgStatusDE.LoadFromResource(AfxGetInstanceHandle(), IDB_ATO_DE);
	imgStatusFA.LoadFromResource(AfxGetInstanceHandle(), IDB_ATO_FA);
...
void CTabDlg1::SetSessionStatusResetAll()
{
	for (int i = 0; i < NUMBER_MAX_SESSION; i++)
	{
...
void CTabDlg1::OnTcnSelchangeTabTs(NMHDR* pNMHDR, LRESULT* pResult)
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	...
	case 0:
		...
		break;
	case 1:
		p01Tab->ShowWindow(SW_HIDE);
		p02Tab->ShowWindow(SW_SHOW);
		p04Tab->ShowWindow(SW_HIDE);
		p07Tab->ShowWindow(SW_HIDE);
		p09Tab->ShowWindow(SW_HIDE);
		p10Tab->ShowWindow(SW_HIDE);
...
void CTabDlg1::OnDestroy()
{
	...
	p01Tab->DestroyWindow();
	delete p01Tab;

	p02Tab->DestroyWindow();
	delete p02Tab;

	p04Tab->DestroyWindow();
	delete p04Tab;

	p07Tab->DestroyWindow();
	delete p07Tab;

	p09Tab->DestroyWindow();
	delete p09Tab;

	p10Tab->DestroyWindow();
	delete p10Tab;
...
void CTabDlg1::OnBnClickedButtonStart()
{
	OnEnKillfocuscop();
	OnEnKillfocuscen();
	OnEnKillfocuscmav();
	...
	CMFCPropertyGridProperty* tempGrid;
	CMFCPropertyGridProperty* currentProperty;
	tempGrid = p01Tab->m_propgrid.GetProperty(BODY_DATA);
...
void CTabDlg1::StartTimer(BOOL bStart)
{	
...
LRESULT CTabDlg1::LogFileAdd(WPARAM wParam, LPARAM lParam)
{	
	m_lc_msg.SaveLogFileAppend(TS_1);
	return TRUE;
}

CString CTabDlg1::getExePath()
{
	CString strPath;
...
unsigned int CTabDlg1::KR37_set_crc32_init_value(unsigned int start)
{
	unsigned int init = 0;
	unsigned int byte;
...
unsigned int CTabDlg1::KR37_compute_check_sum(byte* start, unsigned int data_len)
{
...
void CTabDlg1::OnBnClickedButtonFinish()
{
	m_bATORunning = FALSE;
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_pic_onoff.SetBitmap(imgDisConn);
...
void CTabDlg1::CopySelectedItemToClipboard(CListCtrl& listCtrl) {
	...
	if (selectedIndex != -1) {
		...
		if (OpenClipboard()) {
			...
			if (hClipboardData) {
				...
				// Set clipboard data
				SetClipboardData(CF_TEXT, hClipboardData); // CF_TEXT for multi-byte text
			}
...
void CTabDlg1::OnNMClickListMsg(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	//type별로 NID_PACKET_ATO 정리1
		...
		else if (m_lc_msg.GetItemText(clickindex, 2) == "HS Ack")
		{
			m_msgType = aHSAck;						// NID_PACKET_ATO : 1 = HandShake Ack
			chgCmb(m_msgType);
			OnCbnSelchangeCmbRecv();
			updtPacket(arr, len);
		...
		else if (m_lc_msg.GetItemText(clickindex, 2) == "JP")
		{
			m_msgType = aJP;						// NID_PACKET_ATO : 4 = Journey Profile
			chgCmb(m_msgType);
			OnCbnSelchangeCmbRecv();
			updtPacket(arr, len);
		...
		else if (m_lc_msg.GetItemText(clickindex, 2) == "SP")
		{
			m_msgType = aSP;						// NID_PACKET_ATO : 8 = Segment Profile
			chgCmb(m_msgType);
			OnCbnSelchangeCmbRecv();
			updtPacket(arr, len);
		...
		else if (m_lc_msg.GetItemText(clickindex, 2) == "STR Ack")
		{
			m_msgType = aSTRAck;						// NID_PACKET_ATO : 9 = Status Report Ack
			chgCmb(m_msgType);
			OnCbnSelchangeCmbRecv();
			updtPacket(arr, len);
		}
...
void CTabDlg1::OnBnClickedButtonCon()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	GetDlgItem(IDC_BUTTON_CON)->EnableWindow(FALSE);
	GetDlgItem(IDC_BUTTON_DISCON)->EnableWindow(TRUE);
...
void CTabDlg1::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
...
void CTabDlg1::OnBnClickedButtonDiscon()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	Cmd_Send(2);
...
void CTabDlg1::OnLvnBeginlabeleditListMsg(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CTabDlg1::OnContextMenu(CWnd* /*pWnd*/, CPoint /*point*/)
{
	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
}

void CTabDlg1::OnRclickListMsg(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CPoint point;
...
void CTabDlg1::OnLvnItemchangedListMsg(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CTabDlg1::OnEnKillfocuscop()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CMFCPropertyGridProperty* tempGrid;
	CMFCPropertyGridProperty* currentProperty;
	...
	Utility::INIWriteString("TS-01", "NID_OPERATIONAL", op, myPath);

	tempGrid = p01Tab->m_propgrid.GetProperty(HEADER);
	...
	tempGrid = p02Tab->m_propgrid.GetProperty(HEADER);
	currentProperty = tempGrid->GetSubItem(IDX_NID_OPERATIONAL);
	Utility::SetValueWithTypeCasting(currentProperty, GetDlgItemInt(IDC_cOP));

	tempGrid = p04Tab->m_propgrid.GetProperty(HEADER);
	currentProperty = tempGrid->GetSubItem(IDX_NID_OPERATIONAL);
	Utility::SetValueWithTypeCasting(currentProperty, GetDlgItemInt(IDC_cOP));

	tempGrid = p07Tab->m_propgrid.GetProperty(HEADER);
	currentProperty = tempGrid->GetSubItem(IDX_NID_OPERATIONAL);
	Utility::SetValueWithTypeCasting(currentProperty, GetDlgItemInt(IDC_cOP));

	tempGrid = p09Tab->m_propgrid.GetProperty(HEADER);
	currentProperty = tempGrid->GetSubItem(IDX_NID_OPERATIONAL);
	Utility::SetValueWithTypeCasting(currentProperty, GetDlgItemInt(IDC_cOP));

	tempGrid = p10Tab->m_propgrid.GetProperty(HEADER);
	currentProperty = tempGrid->GetSubItem(IDX_NID_OPERATIONAL);
	Utility::SetValueWithTypeCasting(currentProperty, GetDlgItemInt(IDC_cOP));

}

void CTabDlg1::OnEnKillfocuscen()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CMFCPropertyGridProperty* tempGrid;
	CMFCPropertyGridProperty* currentProperty;
	...
	Utility::INIWriteString("TS-01", "NID_ENGINE", en, myPath);
...
void CTabDlg1::OnEnKillfocuscmav()
{
	
	CString ver;
	m_cMAV.GetWindowTextA(ver);
	Utility::INIWriteString("TS-01", "VERSION", ver, myPath);
...
BOOL CTabDlg1::PreTranslateMessage(MSG* pMsg)
{
...
void CTabDlg1::OnBnClickedButtonAdd()
{	
	vector<byte> logVector;
	CString inputStr, csCommand, csType;
	GetDlgItemText(IDC_EDIT_LOG, inputStr);

	if (inputStr.IsEmpty())
	{
		AfxMessageBox(_T("입력 문자열이 비어 있습니다."), MB_OK | MB_ICONWARNING);
		return;
	}	

	if (inputStr.GetLength() < 30)
	{
		AfxMessageBox(_T("유효한 길이의 패킷이 아닙니다."), MB_OK | MB_ICONWARNING);
		return;
	}

	logVector = Utility::tokenizeString16(inputStr);
		
	csType = getPacketString(logVector[8]);
	MessageLogVector(_T("Test"), csCommand, csType, logVector);
}


void CTabDlg1::OnBnClickedButtonClear()
{
	SetDlgItemText(IDC_EDIT_LOG, _T(""));
}


void CTabDlg1::OnBnClickedCheckIndi()
{
	//StartTimer(!m_checkindi.GetCheck());
