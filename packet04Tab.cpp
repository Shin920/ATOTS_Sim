...
IMPLEMENT_DYNAMIC(packet04Tab, CDialogEx)

packet04Tab::packet04Tab(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DLG_SEND_3, pParent), pJPStructure(nullptr), pFileJPStructure(nullptr)
{
	Init();
}

packet04Tab::~packet04Tab()
{
	Release();
}

void packet04Tab::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(packet04Tab, CDialogEx)
	ON_WM_ERASEBKGND()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_CHB_AUTO, &packet04Tab::OnBnClickedChbAuto)
	ON_CBN_SELCHANGE(IDC_CMB_PACKET, &packet04Tab::OnCbnSelchangeCmbPacket)
	ON_BN_CLICKED(IDC_BTN_SEND, &packet04Tab::OnBnClickedBtnSend)
	ON_BN_CLICKED(IDC_BTN_SAVE, &packet04Tab::OnBnClickedBtnSave)
	ON_BN_CLICKED(IDC_BTN_LOAD, &packet04Tab::OnBnClickedBtnLoad)
	ON_BN_CLICKED(IDC_BUTTON1, &packet04Tab::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BTN_SEND_DYNAMIC, &packet04Tab::OnBnClickedBtnSendDynamic)
END_MESSAGE_MAP()


// CSendTab 메시지 처리기

void packet04Tab::Init()
{
...
void packet04Tab::PacketHeaderCtrlToStructure(const CMFCPropertyGridProperty* bodyProperty, ATO_Header& p)
{
...
void packet04Tab::PacketJPCtrlToStructure(const CMFCPropertyGridProperty* bodyProperty, TS_OB_Packet04_JPStruct& p)
{
...
void packet04Tab::PacketJPCtrlToStructureDynamic(const CMFCPropertyGridProperty* bodyProperty, TS_OB_Packet04_JPStruct& p)
{
	...
	for (int iSP = 0; iSP < (int)p.N_ITER_SP; ++iSP)
	{
		...
		p.segmentProfileReference[iSP].NID_C = (UINT16)iterSPNJPProperty->GetSubItem(IDX_SPNJP_NID_C)->GetValue().intVal;
		//p.segmentProfileReference[iSP].NID_SP = (UINT32)iterSPNJPProperty->GetSubItem(IDX_SPNJP_NID_SP)->GetValue().intVal;
		p.segmentProfileReference[iSP].NID_SP = _UINT32_GETVALUE_(iterSPNJPProperty, IDX_SPNJP_NID_SP);
		p.segmentProfileReference[iSP].M_SP_Version = (UINT16)iterSPNJPProperty->GetSubItem(IDX_SPNJP_M_SP_Version)->GetValue().intVal;
		p.segmentProfileReference[iSP].Q_SP_DIR = (UINT8)iterSPNJPProperty->GetSubItem(IDX_SPNJP_Q_SP_DIR)->GetValue().intVal;
		p.segmentProfileReference[iSP].N_ITER = (UINT8)iterSPNJPProperty->GetSubItem(IDX_SPNJP_N_ITER_TP)->GetValue().intVal;
...
BOOL packet04Tab::OnEraseBkgnd(CDC* pDC)
{
...
HBRUSH packet04Tab::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void packet04Tab::OnBnClickedChbAuto()
{
...
void packet04Tab::OnCbnSelchangeCmbPacket()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void packet04Tab::OnBnClickedBtnSend()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	// Sa-Conn 연결여부 확인 필요
...
void packet04Tab::PacketJPStructrureToINI(TS_OB_Packet04_JPStruct& p)
{
	if (&p == nullptr)return;
...
BOOL packet04Tab::LoadPacketJPFromFile()
{
...
BOOL packet04Tab::initLoadPacketJPFromFile()
{
	TS_OB_Packet04_JPStruct& p = *pFileJPStructure;
	if (&p == nullptr)return FALSE;
	static CString pathName = JPDefaultConfigFileName;
...
void packet04Tab::LoadPacketJPFromFile_StaticData(string& configTXT, TS_OB_Packet04_JPStruct& p)
{
	//size_t strIdx, strEndlineIdx;
	//string currentLine;
...
void packet04Tab::ReLoadPacketJP()
{
...
void packet04Tab::OnBnClickedBtnSave()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void packet04Tab::OnBnClickedBtnLoad()
{
...
int packet04Tab::getPacketCounter()
{
	m_nPacketCounter++;
...
void packet04Tab::OnBnClickedButton1()
{
...
BOOL packet04Tab::PreTranslateMessage(MSG* pMsg)
{
...
void packet04Tab::OnBnClickedBtnSendDynamic()
{
	CTabDlg1* targetDlg = ((CATOTSSimulatortabDlg*)AfxGetMainWnd())->pDlg1;
