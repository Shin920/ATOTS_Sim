...
IMPLEMENT_DYNAMIC(packet09Tab, CDialogEx)

packet09Tab::packet09Tab(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DLG_SEND_5, pParent), pSTRAckStructure(nullptr)
{
	Init();
}

packet09Tab::~packet09Tab()
{
	Release();
}

void packet09Tab::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(packet09Tab, CDialogEx)
	ON_WM_ERASEBKGND()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_CHB_AUTO, &packet09Tab::OnBnClickedChbAuto)
	ON_CBN_SELCHANGE(IDC_CMB_PACKET, &packet09Tab::OnCbnSelchangeCmbPacket)
	ON_BN_CLICKED(IDC_BTN_SEND, &packet09Tab::OnBnClickedBtnSend)
	ON_BN_CLICKED(IDC_BTN_SAVE, &packet09Tab::OnBnClickedBtnSave)
	ON_BN_CLICKED(IDC_BTN_LOAD, &packet09Tab::OnBnClickedBtnLoad)
END_MESSAGE_MAP()


// CSendTab 메시지 처리기

void packet09Tab::Init()
{
...
void packet09Tab::PacketHeaderCtrlToStructure(const CMFCPropertyGridProperty* bodyProperty, ATO_Header& p)
{
...
void packet09Tab::PacketSTRAckCtrlToStructure(const CMFCPropertyGridProperty* bodyProperty, TS_OB_Packet09_STRAckStruct& p)
{
	const CMFCPropertyGridProperty& cp = *(bodyProperty);
...
BOOL packet09Tab::OnEraseBkgnd(CDC* pDC)
{
...
HBRUSH packet09Tab::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void packet09Tab::OnBnClickedChbAuto()
{
...
void packet09Tab::OnCbnSelchangeCmbPacket()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void packet09Tab::OnBnClickedBtnSend()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	// Sa-Conn 연결여부 확인 필요
...
void packet09Tab::PacketSTRAckStructrureToINI(TS_OB_Packet09_STRAckStruct& p)
{
	if (&p == nullptr)return;
...
BOOL packet09Tab::LoadPacketSTRAckFromFile()
{
...
BOOL packet09Tab::initLoadPacketSTRAckFromFile()
{
	TS_OB_Packet09_STRAckStruct& p = *pFileSTRAckStructure;
	if (&p == nullptr)return FALSE;
	static CString pathName = STRAckDefaultConfigFileName;
...
void packet09Tab::LoadPacketSTRAckFromFile_StaticData(string& configTXT, TS_OB_Packet09_STRAckStruct& p)
{
	size_t strIdx, strEndlineIdx;
	string currentLine;
...
void packet09Tab::ReLoadPacketSTRAck()
{
...
void packet09Tab::OnBnClickedBtnSave()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void packet09Tab::OnBnClickedBtnLoad()
{
...
int packet09Tab::getPacketCounter()
{
	m_nPacketCounter++;
...
BOOL packet09Tab::PreTranslateMessage(MSG* pMsg)
{
